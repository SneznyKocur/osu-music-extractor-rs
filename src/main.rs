...
fn parse_osu(file: &DirEntry)  -> OsuFile{
    ...
    let mut osufile: OsuFile = OsuFile { artist: String::new(), title: String::new(), sound_path: PathBuf::new() };
...
fn copy(file_path: &DirEntry,output_dir: &Path)  -> () {
    if !file_path.file_type().unwrap().is_file() {return}
    ...
    if !(extension == "osu") {return}
    let osu_file: OsuFile = parse_osu(file_path);
    let sound_extension: &str = match osu_file.sound_path.extension() {
        Some(extension) => extension.to_str().unwrap(),
        None => {
            eprintln!("Sound file {:#?} does not have an extension!",osu_file.sound_path);
            return;
        }
    };
    let output_file_name = &output_dir.join(escape_string(&osu_file.title.clone()) + " - " + &escape_string(&osu_file.artist.clone()) + "." + sound_extension);
...
fn add_tag(file_path: &PathBuf, osu_file: OsuFile) {
    let mut tag = match Tag::new().read_from_path(file_path) {
        Ok(tag) => tag,
        Err(_) => {
            Box::new(audiotags::Id3v2Tag::default())
        }
    };
...
fn main() -> std::io::Result<()> {
    ...
    let _ = stdout().flush();
    stdin().read_line(&mut songs_directory).expect("Failed to get user input!");
    print!("Enter output songs directory (will be created if it does not exist):");
    let _ = stdout().flush();
    stdin().read_line(&mut output_directory).expect("Failed to get user input!");
